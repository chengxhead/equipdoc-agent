...
脚本应完成一次预热和3次正式请求，并输出 `summary`。高保证回答守卫要求每条技术陈述逐字摘录一条检索证据，并在句末带有该证据引用；未通过时只重试一次，再失败则返回带逐句引用的抽取式证据。结果会分别记录首轮通过、重试恢复和抽取式降级，避免把系统兜底伪装成模型质量。
...
- 引用原文逐字匹配证明返回的技术句来自所引切片，但不能证明知识切片本身正确或足够权威；
