...
脚本应完成一次预热和3次正式请求，并输出 `summary`。高保证回答链路先根据问题中的明确设备/故障词进行聚焦检索，再让 Qwen 从候选证据句中选择2至5个ID，最终文本与引用由系统确定性渲染。若模型没有返回合格ID则重试一次，再失败才按词法相关性返回最多5条原文。结果分别记录首轮选择成功、重试恢复和抽取式降级，避免把系统兜底伪装成模型能力。
