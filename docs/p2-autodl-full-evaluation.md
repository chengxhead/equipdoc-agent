...
脚本应完成一次预热和3次正式请求，并输出 `summary`。逐句引用守卫要求每条技术陈述在句末带有直接证据引用；未通过时只重试一次，再失败则返回带逐句引用的抽取式证据。结果会分别记录首轮通过、重试恢复和抽取式降级，避免把系统兜底伪装成模型生成质量。
...
- 自动通过只表示关键词、禁止词、引用ID和逐句引用覆盖规则通过，不等于人工正确率；
- 逐句引用覆盖只表示每条陈述带有可回查引用，不能自动证明陈述与证据语义完全一致；
