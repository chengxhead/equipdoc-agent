...
- 请求成功率和自动用例通过率；
- 必需关键词平均召回率（单题必须全部命中才计为通过）；
- 禁止性具体结论规避率；
- 引用ID有效率、逐句引用覆盖率、引用原文逐字匹配率与参考文档命中率；
- 串行端到端平均延迟、p50和p95；
...
脚本应完成一次预热和3次正式请求，并输出 `summary`。高保证回答链路先根据问题中的明确设备/故障词进行聚焦检索，再按频谱分析、现场复核等问题意图重排和去冗余，然后让 Qwen 选择4个证据句ID，最终文本与引用由系统确定性渲染。若模型没有返回合格ID则重试一次，再失败才按词法相关性返回最多5条原文。结果分别记录首轮选择成功、重试恢复和抽取式降级，避免把系统兜底伪装成模型能力。

如果出现连续3次服务错误，脚本会提前停止，避免等待20次超时。完整评测前应人工查看3条回答，确认没有自动规则未覆盖的专业错误。
...
- 自动通过只表示关键词、禁止词、引用ID、逐句引用覆盖和引用原文逐字匹配规则通过，不等于人工正确率；
- 引用原文逐字匹配证明返回的技术句来自所引切片，但不能证明模型选择了最完整的证据，也不能证明知识切片本身正确或足够权威；
